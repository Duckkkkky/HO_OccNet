...
from utils.sdf_utils import pixel_align
from datasets.dexycb.toolkit.dex_ycb import _SUBJECTS, _SERIALS


def reconstruct(cfg, filename_frames, model, latent_vec_frames, metas_frames, hand_pose_results_frames, obj_pose_results_frames):
        ...
        while head < num_samples:
            ...
            if cfg.hand_encode_style == 'kine':
                if cfg.test_with_gt:
                    ...
                    _, _, _, gt_global_trans, gt_rot_center = mano_layer(meta['hand_poses'], th_betas=metas['hand_shapes'], root_palm=False)
            ...
            # latent_vec_refined = latent_vec_refined.reshape(cfg.test_batch_size, sample_subset.shape[0], -1)
            # latent_vec_refined = latent_vec_refined.mean(1)
            latent_vec_refined = model.module.sdf_encoder(latent_vec_refined)
            # latent_vec_refined = latent_vec_refined.repeat_interleave(sample_subset.shape[0], dim=0)
